...
const addContent = async function (tocDetails, reportId, sectionDetails) {
  ...
  for (let i = 0; i < oTocDetails.length; i++) {
    ...
    // Prepare the partial update object (only set provided fields)
    const updateObj = {};
    if (!utilities.isEmpty(element.content)) updateObj['toc.$.content'] = element.content;
    if (!utilities.isEmpty(element.section_name)) updateObj['toc.$.section_name'] = element.section_name;
    if (!utilities.isEmpty(element.meta)) updateObj['toc.$.meta'] = element.meta;

    // Try to update an existing toc entry matching section_id
    const updateResult = await reportModel.Reports.updateOne(
      { _id: mongoose.Types.ObjectId(reportId), 'toc.section_id': element.section_id },
      { $set: updateObj }
    );

    // If no existing entry was matched, push the whole element into the toc array
    if (!updateResult.matchedCount || updateResult.matchedCount === 0) {
      const pushRes = await reportModel.Reports.updateOne(
