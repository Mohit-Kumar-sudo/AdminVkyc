...
async function saveImagesForPatient(patientId, files) {
  ...
  for (const file of files) {
    const ts = Date.now();
    const outName = `${ts}-${file.originalname.replace(/\s+/g, '_')}.jpg`;
    ...
    const relPath = path.join('uploads', 'patients', patientId, outName);
...
async function addBeforeAfterImages(req, res) {
  try {
    ...
    for (const file of req.files) {
      const ts = Date.now();
      const outName = `${type}-${ts}-${file.originalname.replace(/\s+/g, '_')}.jpg`;
      const outPath = path.join(baseDir, outName);
      const pipeline = sharp(file.buffer).rotate().resize({ width: 1280, withoutEnlargement: true }).jpeg({ quality: 85 });
      const { width, height, size } = await pipeline.toFile(outPath);
      const relPath = path.join('uploads', 'patients', patient._id.toString(), 'before-after', outName);
    ...
    // Return the uploaded image path for generation API
    return res.json({ 
      ...patient.toObject(), 
