...
async function googleLogin(req, res) {
  try {
    ...
    if (!user) {
      // Create a random password so password field is populated
      const tempPassword = crypto.randomBytes(16).toString('hex');
      const passwordHash = await hashPassword(tempPassword);
      user = await User.create({ name: name || email.split('@')[0], email, googleId, password: passwordHash, photoUrl: picture });
    } else {
      if (!user.googleId) {
