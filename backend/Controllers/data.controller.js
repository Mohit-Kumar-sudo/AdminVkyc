...
module.exports = {
  getDeedCategory: async (req, res, next) => {
    try {
      axios
        .get(
          "https://sushrut-coe.mp.gov.in/sampadaService/common/duty/allDeedCategory",
          {
            params: {},
          }
        )
  ...
  getAllDeedTypeByCategoryId: async (req, res, next) => {
    try {
      ...
      axios
        .get(
          `https://sushrut-coe.mp.gov.in/sampadaService/common/duty/getAllDeedTypeByCategoryId/${id}`,
          {}
        )
        .then(
          function (response) {
  ...
  deedInstruments: async (req, res, next) => {
    try {
      ...
      axios
        .get(
          `https://sushrut-coe.mp.gov.in/sampadaService/common/duty/deedInstruments/${id}`,
          {}
        )
        .then(
          function (response) {
  ...
  partyRoles: async (req, res, next) => {
    try {
      ...
      axios
        .get(
          `https://sushrut-coe.mp.gov.in/sampadaService/common/duty/getAllPartyTypeByInstrumentId/${id}`,
          {}
        )
        .then(
          function (response) {
  ...
  userAuthenticate: async (req, res, next) => {
    try {
      ...
      axios
        .post(
          `https://sushrut-coe.mp.gov.in/sampadaGateway/common/authenticate_redirect_department_user`,
          { uid: uid }
        )
        .then(
          function (response) {
            ...
            if (token) {
              ...
              return res.redirect(
                `http://10.115.206.80/admin/#/Authenticate/${uid}/${otp}`
              );
            } else {
