...
const UserSchema = new Schema(
  {
    ...
    myPpts: [
      {
        ...
        reportId: {type: Schema.Types.ObjectId, ref: 'reports', trim: true, required: true},
        slides: [
    ...
    reportIds: [
      {
        type: Schema.Types.ObjectId, ref: 'reports', trim: true, required: true
      }
    ],
    featuredReportIds: [
      {
...
const emailConfirm = async function (loginId) {
  return await Users.updateOne({"_id": mongoose.Types.ObjectId(loginId)}, {"emailConfirm": 1}, {upsert: true})
}

const getEmailConfirm = async function (loginId) {
  const query = Users.findOne({"_id": mongoose.Types.ObjectId(loginId)}, 'emailConfirm');
  return (query.lean().exec({"virtuals": true}));
};


const getUserFeaturedReports = (user) => {
  const query = Users.findOne({"_id": mongoose.Types.ObjectId(user._id)}, 'featuredReportIds');
  return (query.lean().exec({"virtuals": true}));
}

const addDashboard = async function (data, userId) {
  return await Users.update({"_id": mongoose.Types.ObjectId(userId)},
...
const getUserDashboards = async function (userId, dashboardId) {
  ...
  if (dashboardId) {
    ...
    query = Users.findOne({"_id": mongoose.Types.ObjectId(userId)}, 'myDashboards');
  }
  return (query.lean().exec({"virtuals": true}));
};

const deleteUserDashboard = async (userId, dashboardId) => {
  return Users.update({_id: mongoose.Types.ObjectId(userId)},
    {
      $pull: {
        myDashboards: {
...
const getUserPPTs = async function (userId) {
  const query = Users.findOne({"_id": mongoose.Types.ObjectId(userId)}, 'myPpts');
  return (query.lean().exec({"virtuals": true}));
};
...
const addSlideToPPT = async (userId, pptId, slideData) => {
  return Users.update(
    {"_id": mongoose.Types.ObjectId(userId), "myPpts._id": mongoose.Types.ObjectId(pptId)},
    {
...
const allUsers = async function (key) {
  let query;
  if (key == 0 || key == 1)
    query = Users.find().select('_id firstName lastName email adminConfirm emailConfirm').where({"adminConfirm": key})
  else
    query = Users.find().select('_id firstName lastName email adminConfirm emailConfirm')
...
const adminConfirm = async function (key, ids) {
  if (key == 1) {
    return await Users.updateMany({_id: {$in: ids}}, {"adminConfirm": 1, "emailConfirm": 1}, {
      upsert: true,
      multi: true
    })
  } else if (key == 0) {
    return await Users.updateMany({_id: {$in: ids}}, {"adminConfirm": 0}, {upsert: true, multi: true})
  }
}
...
const getUserSelfServiceReports = async function (userId) {
  const query = Users.findOne({"_id": mongoose.Types.ObjectId(userId)}, 'mySelfServiceReports');
  return (query.lean().exec({"virtuals": true}));
};

const getSelfReportData = async function (userId, id) {
  return await Users.findOne({"mySelfServiceReports._id": mongoose.Types.ObjectId(id)}, 'mySelfServiceReports').lean().exec({"virtuals": true})
}

const updateSelfReportData = async (userId, ssrId, data) => {
  return Users.update(
    {"_id": mongoose.Types.ObjectId(userId), "mySelfServiceReports._id": mongoose.Types.ObjectId(ssrId)},
...
const assignReports = async (userId, body) => {
  return Users.update(
    {"_id": mongoose.Types.ObjectId(userId)},
    {"$set": {"reportIds": body.reportIds, "featuredReportIds": body.featuredReportIds}},
