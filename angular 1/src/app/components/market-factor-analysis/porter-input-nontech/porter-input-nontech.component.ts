...
@Component({
  selector: 'app-porter-input-nontech',
  templateUrl: './porter-input-nontech.component.html',
  styleUrls: ['./porter-input-nontech.component.scss']
})
export class PorterInputNonTechComponent implements OnInit {
  ...
  getPortersDataSuccess(data) {
    ...
    if(data && data.data && data.data[0] && data.data[0].toc && data.data[0].toc.meta) {
      if (data.data[0].toc.meta.data) {
      ...
      if(data.data[0].toc.meta.impact){
        this.impactInput = data.data[0].toc.meta.impact
      }
