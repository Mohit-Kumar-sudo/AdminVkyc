...
bulkUser: async (req, res) => {
  try {
    ...
    // Convert Excel row to usable object
    ...
    // Collect all values in uploaded file
    const fileSNOS = newUsers.map(u => u.sNo);
    const fileMobiles = newUsers.map(u => u.mobile);
    const fileEmails = newUsers.map(u => u.email);
    ...
    const existingSNOs = new Set(existing.map(e => e.sNo));
    const existingMobiles = new Set(existing.map(e => e.mobile));
    const existingEmails = new Set(existing.map(e => e.email));

    // Check duplicates within Excel file itself
    const fileDuplicateSNO = new Set(fileSNOS.filter((v, i, a) => a.indexOf(v) !== i));
    const fileDuplicateMobile = new Set(fileMobiles.filter((v, i, a) => a.indexOf(v) !== i));
    const fileDuplicateEmail = new Set(fileEmails.filter((v, i, a) => a.indexOf(v) !== i));

    // Loop & validate each user
    for (const u of newUsers) {
      let reason = "";

      // DB duplicates
      if (existingSNOs.has(u.sNo)) reason += "Duplicate sNo in database. ";
      if (existingMobiles.has(u.mobile)) reason += "Duplicate mobile in database. ";
      if (existingEmails.has(u.email)) reason += "Duplicate email in database. ";

      // File duplicates
      if (fileDuplicateSNO.has(u.sNo)) reason += "Duplicate sNo in file. ";
      if (fileDuplicateMobile.has(u.mobile)) reason += "Duplicate mobile in file. ";
      if (fileDuplicateEmail.has(u.email)) reason += "Duplicate email in file. ";

      // If any duplicate → skip user
      if (reason) {
        skippedUsers.push({
          sNo: u.sNo,
          mobile: u.mobile,
