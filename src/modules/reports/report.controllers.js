...
export async function getIndustryReport(req, res) {
  try {
          ...
          for (const item of o.items) {
            for (const toc of data.toc) {
              ...
              if (regex.test(toc.section_name)) {
                content.push({
                  label: item.label,
                  data: toc.content[0].data.length ? toc.content[0].data[0].info.slice(0, 2000) : toc.content[0].data.content.slice(0, 2000)
                });
              }
            }
          }
    ...
    if (!utilities.isEmpty(data.errors)) {
      ...
      utilities.sendResponse(HTTPStatus.OK, newData, res);
    }
  } catch (er) {
    utilities.sendErrorResponse(
      HTTPStatus.INTERNAL_SERVER_ERROR,
