...
const fetchReport = function(
  ...
  if (reportId && vertical == null && !selectKeys) {
    ...
    if (reportId) {
      ...
      query.where({
        searching_title: {
          $regex: `\\b${getReportNameString(reportName)}\\b`,
          $options: "i"
        }
      });
    }
    ...
    if (reportId) {
      ...
      query.push({
        $match: {
          searching_title: {
            $regex: new RegExp(`\\b${getReportNameString(reportName)}\\b`, "i")
          }
        }
      });
    }
    ...
    const queryPromise = Reports.aggregate(query).allowDiskUse();
