...
const getChartData = async (reportId, chartSearchName) => {
  const initialMatchQuery = {
    ...(!utilities.isEmpty(reportId) && {
      _id: mongoose.Types.ObjectId(reportId)
    }),
    $or: [
      { "toc.content.type": { $in: ["CHART", "PIE", "BAR"] } },
      { "toc.content.data.type": { $in: ["CHART", "PIE", "BAR", "IMAGE"] } }
    ]
  };
  ...
  const contentMatchQuery = {
    $and: [
      {
        $or: [
          { "toc.content.data.type": { $in: ["CHART", "PIE", "BAR"] } },
          { "toc.content.type": { $in: ["CHART", "PIE", "BAR", "IMAGE"] } },
          {
