...
startTimer(): void {
    ...
    // Use sale end date if available, otherwise default to 24 hours
    const endTime = this.saleEndDate || (() => {
        const defaultEnd = new Date();
        defaultEnd.setHours(defaultEnd.getHours() + 24);
        console.warn('No sale end date found, using 24-hour default');
        return defaultEnd;
    })();

    console.log('Timer countdown to:', endTime);
...
private updateTimerDisplay(endTime: Date): void {
    ...
    if (distance < 0) {
        ...
        console.log('Sale timer expired');
        return;
    }
...
onBuyCourse(): void {
    ...
    // Determine sale end date
    let saleEndDate = null;
    if (this.course.sale_end_date) {
        saleEndDate = new Date(this.course.sale_end_date);
    } else if (hasSale) {
        // Default to 24 hours from now if no end date specified
...
setupSaleTimer(): void {
    ...
    if (this.hasSale && this.course.sale_end_date) {
        // Parse the sale end date from backend
        this.saleEndDate = new Date(this.course.sale_end_date);
        const now = new Date();
        
        // Validate the date
        if (isNaN(this.saleEndDate.getTime())) {
            console.error('Invalid sale end date:', this.course.sale_end_date);
            this.hasSale = false;
            this.isSaleActive = false;
            return;
        }
        ...
        if (this.saleEndDate > now) {
            ...
            console.log('Sale timer started. Ends at:', this.saleEndDate);
        } else {
            // Sale has already ended
            console.log('Sale has already ended');
            this.isSaleActive = false;
            this.hasSale = false;
        }
    } else {
