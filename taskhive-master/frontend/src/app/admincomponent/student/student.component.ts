...
data: any;
  ...
  ngOnInit() {
    ...
    fetchAllStudents() {
    this.studentService.get('students',{}).subscribe(res => {
      this.getData = res;
  ...
  editStudent(student: any, content: any) {
  ...
  onDelete(id: any) {
    ...
    modalRef.result.then((result) => {
      if (result === 'Delete' && this.studentToDeleteId) {
        this.studentService.delete('students',this.studentToDeleteId).subscribe(res => {
          console.log(res);
          this.fetchAllStudents();
        });
      }
  ...
  handleFileInput(event: any) {
    this.selectedStudent.image = event.target.files[0]; 
  }
      ...
      this.fileServ.uploadFile(this.image[0]).subscribe(
        (res: any) => {
          if (res.type === HttpEventType.Response) {
            ...
            this.selectedStudent.image = imagePath;
            console.log('students', this.selectedStudent)
            this.studentService.put('students',this.selectedStudent._id, this.selectedStudent).subscribe(res => {
              this.fetchAllStudents();
              modal.close(); 
            });
          }
