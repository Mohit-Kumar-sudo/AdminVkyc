...
@Component({
  ...
  styleUrl: './add-testimonial.component.scss' // keep same key as your original file
})
export class AddTestimonialComponent implements OnInit, OnDestroy {
  myForm!: FormGroup;
  isLoading = false;
  ...
  onSubmit(frm: FormGroup) {
    ...
    this.postSub = this.api.post('testimonials', this.myForm.value).pipe(
      timeout(10000),
      catchError(err => {
        return throwError(err);
      }),
      ...
      (res: any) => {
        if (res) {
          this.as?.successToast?.('Testimonial created successfully!');
          alert('Testimonial created successfully!');
          this.route.navigate(['/admin/testimonials']);
        } else {
          this.as?.warningToast?.(res?.error?.message || 'Error creating testimonial');
        }
      },
      (error) => {
        const errMsg = (error?.name === 'TimeoutError') ?
          'Server is taking too long to respond. Please try again later.' :
          (error?.message || 'Error creating testimonial');
