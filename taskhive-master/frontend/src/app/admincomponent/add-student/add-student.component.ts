...
myForm!: FormGroup;
  ...
  constructor(
    ...
    this.myForm = this.fb.group({
      ...
      college: ['', [Validators.required, Validators.maxLength(1500)]],
    });
  }
  ...
  onSubmit(frm: FormGroup) {
    ...
    this.postSub = this.api.post('students', this.myForm.value).pipe(
      timeout(10000),
      catchError(err => {
        return throwError(err);
      }),
      ...
      (res: any) => {
        if (res) {
          this.as?.successToast?.('Student Card Added successfully!');
          // keep the browser alert if you like — toasts are preferable
          alert('Students Card Added successfully!');
          this.route.navigate(['/admin/students']);
        } else {
          this.as?.warningToast?.(res?.error?.message || 'Error creating student card');
        }
      },
      (error) => {
        const errMsg = (error?.name === 'TimeoutError') ?
          'Server is taking too long to respond. Please try again later.' :
          (error?.message || 'Error creating Student card');
