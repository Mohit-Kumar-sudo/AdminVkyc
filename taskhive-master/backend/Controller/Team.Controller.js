...
module.exports = {
  ...
  delete: async (req, res, next) => {
    try {
        ...
        const Team = await Model.findById(new mongoose.Types.ObjectId(id)); // Use new here
        if (!Team) {
            return res.status(404).json({ message: 'Team not found' });
        }

        // Mark the Team as inactive instead of deleting
        ...
        res.json({ success: true, message: 'Team marked as inactive', data: result });
