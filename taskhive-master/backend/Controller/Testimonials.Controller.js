...
const Model = require('../Models/Testimonials.Model')
...
module.exports = {
  ...
  delete: async (req, res, next) => {
    try {
        ...
        const testimonial = await Model.findById(new mongoose.Types.ObjectId(id)); // Use new here
        if (!testimonial) {
            return res.status(404).json({ message: 'testimonial not found' });
        }

        // Mark the testimonial as inactive instead of deleting
        ...
        res.json({ success: true, message: 'testimonial marked as inactive', data: result });
