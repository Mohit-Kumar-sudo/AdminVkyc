...
module.exports = {
  ...
  delete: async (req, res, next) => {
    try {
        ...
        const student = await Model.findById(new mongoose.Types.ObjectId(id)); // Use new here
        if (!student) {
            return res.status(404).json({ message: 'student not found' });
        }

        // Mark the student as inactive instead of deleting
        ...
        res.json({ success: true, message: 'student marked as inactive', data: result });
