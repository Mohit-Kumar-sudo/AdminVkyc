...
module.exports = {
  ...
  try {
    ...
    // --- sanitize payload: accept partial input, but remove empty strings ---
    const allowedFields = [
    ...
    for (const key of allowedFields) {
      ...
      // treat undefined or null or empty-string as "not provided"
      if (v !== undefined && v !== null && !(typeof v === 'string' && v.trim() === '')) {
        // convert numeric fields if necessary
        if (['jobDuration', 'jobExperience', 'jobSalary'].includes(key)) {
          const n = Number(v);
          payload[key] = Number.isFinite(n) ? n : v; // if can't convert keep original
        } else {
          payload[key] = v;
        }
      }
    ...
    // Add metadata
    payload.created_by = req.user ? req.user._id : null;
    payload.updated_by = req.user ? req.user._id : null;
    payload.created_at = new Date();
    payload.updated_at = new Date();

    console.log('Processed payload before save:', JSON.stringify(payload));
